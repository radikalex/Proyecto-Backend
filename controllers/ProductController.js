...
const ProductController = {
    async getProducts(req, res) {
        try {
            ...
            console.error(error);
            res.status(500).send({
                msg: "There was an error getting the products",
                error,
            });
        }
    },
    ...
    async updateProductById(req, res) {
        try {
            ...
            if (product) {
                ...
                res.status(404).send({
                    msg: `Error: No product with id ${req.params.id} found`,
                });
            }
        } catch (error) {
            console.error(error);
    ...
    async getProductById(req, res) {
        try {
            ...
            console.error(error);
            res.status(500).send({
                msg: "There was an error getting a product by id",
                error,
            });
        }
    },
        ...
        try {
            ...
            if (products.length > 0)
                res.send({
                    ...
                    } found with '${req.params.name}' in their name`,
                    results: products,
                });
            else
                res.status(200).send({
            ...
            console.error(error);
            res.status(500).send({
                msg: "There was an error getting a product by name",
                error,
            });
        }
    },
    async getProductsByPrice(req, res) {
        try {
            ...
            console.error(error);
            res.status(500).send({
                msg: "There was an error getting a product by price",
                error,
            });
        }
    },
